...
import {get, fetchOtherProfile , adding ,addNewReview ,deleteReview} from '../../store/other-profile-srore';
		...
		<Accordion>
		    <Card>
			  ...
			  <Card.Body> {props.other.results.reviews.map((rev,idx)=>{ return (
				...
				<Button key={idx+ "delete"} variant="danger" onClick={onClickHanler}>Delete Review</Button>
			      </ul>
			      ) })}
			  </Card.Body>
		        </Accordion.Collapse>
...
const mapDispatchToProps = {get, fetchOtherProfile , adding ,addNewReview  ,deleteReview};
